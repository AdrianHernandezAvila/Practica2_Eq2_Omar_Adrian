...
#define APP_TEAM2_URI_PATH                       "/team2"
...
static void APP_CoapTeam2Cb(coapSessionStatus_t sessionStatus, uint8_t *pData, coapSession_t *pSession, uint32_t dataLen);
...
const coapUriPath_t gAPP_TEAM2_URI_PATH = {SizeOfString(APP_TEAM2_URI_PATH), (uint8_t *)APP_TEAM2_URI_PATH};
...
)
{
    coapRegCbParams_t cbParams[] =  {{APP_CoapLedCb,  (coapUriPath_t *)&gAPP_LED_URI_PATH},
									 ...
									 {APP_CoapTeam2Cb, (coapUriPath_t *)&gAPP_TEAM2_URI_PATH},
...

{
    ...
    if (gCoapConfirmable_c == pSession->msgType)
  {
		if (gCoapGET_c == pSession->code)
		{
			shell_printf("'CON' instruction received from: %s", addrStr);
		}
